  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Google Workspace integration (Gmail, Calendar, Drive). Configured via environment
          variables.
        </CardDescription>
      </CardHeader>
            ...
            <div className="px-4 pb-4 space-y-3">
              <ol className="list-decimal list-inside space-y-2 text-sm text-muted-foreground">
                ...
                <li>
                  ...
                  <strong className="text-foreground">OAuth client ID</strong> and select{' '}
                  <strong className="text-foreground">Web application</strong> as the application
                  type
                </li>
                <li>
                  If prompted, configure the{' '}
                  <strong className="text-foreground">OAuth consent screen</strong> first:
                  <ul className="list-disc list-inside ml-4 mt-1 space-y-1 text-xs">
                    <li>
                      Choose <strong className="text-foreground">External</strong> user type (or
                      Internal for Google Workspace orgs)
                    </li>
                    <li>Fill in app name, support email, and authorized domains</li>
                    <li>
                      Add scopes: <code className="bg-muted px-1 rounded">Gmail API</code>,{' '}
                      <code className="bg-muted px-1 rounded">Calendar API</code>,{' '}
                      <code className="bg-muted px-1 rounded">Drive API</code>
                    </li>
                    <li>
                ...
                <li>
                  ...
                  <code className="bg-muted px-1 rounded">GOOGLE_CLIENT_ID</code> and{' '}
                  <code className="bg-muted px-1 rounded">GOOGLE_CLIENT_SECRET</code> and restart
                  the server
                </li>
              </ol>
          ...
          <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">
                    {testMutation.data.valid
                      ...
                      : testMutation.data.message || 'Invalid credentials'}
                  </span>
                </div>

                {testMutation.data.connectedEmail && (
                  <p className="text-xs text-muted-foreground ml-6">
                    Testing with connected account:{' '}
                    <strong>{testMutation.data.connectedEmail}</strong>
                  </p>
                )}
            ...
            {!data.configured && (
              <p className="text-xs text-muted-foreground">
                ...
                <code className="bg-muted px-1 rounded">GOOGLE_CLIENT_SECRET</code> environment
                variables to enable Google Workspace integration.
              </p>
            )}
          </div>
        )}
