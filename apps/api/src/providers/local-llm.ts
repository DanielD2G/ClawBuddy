...
/**
 * LLM provider for any OpenAI-compatible local server
 * (LM Studio, vLLM, LocalAI, Jan, text-generation-webui, etc.).
 *
 * Uses the OpenAI SDK with a custom baseURL. No API key required.
 * The baseURL should already include /v1 if needed (e.g. http://localhost:1234/v1).
 */
export class LocalLLMProvider implements LLMProvider {
  ...
  constructor(model: string, baseUrl: string) {
    // Ensure baseURL ends with /v1 for the OpenAI SDK
    const normalized = baseUrl.replace(/\/+$/, '')
    this.client = new OpenAI({
      baseURL: normalized.endsWith('/v1') ? normalized : normalized + '/v1',
      apiKey: 'local', // dummy key — local servers don't require auth
    })
    this.model = model
    this.modelId = model
  }
