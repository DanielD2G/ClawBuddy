...
This guide walks you through configuring supported LLM providers, including API-key providers and local OpenAI-compatible backends, plus Google OAuth for Workspace integration.

---

## LLM Provider Connections

You need at least **one** provider connection to use ClawBuddy. Add it to your `.env` file.
...
ClawBuddy can connect to a local OpenAI-compatible endpoint through a single `Local Provider`.
...
> No API key is required for the local provider. ClawBuddy discovers models from `/v1/models`.
...
| ------------------------- | ----------------------- | ------------------------------------------------------------- |
| `AI_PROVIDER`             | Yes                     | Chat model provider: `openai`, `claude`, `gemini`, or `local` |
| `EMBEDDING_PROVIDER`      | Yes                     | Embedding provider: `openai`, `gemini`, or `local`            |
| `OPENAI_API_KEY`          | If using OpenAI         | OpenAI API key                                                |
| `ANTHROPIC_API_KEY`       | If using Claude         | Anthropic API key                                             |
| `GEMINI_API_KEY`          | If using Gemini         | Google AI Studio API key                                      |
| `LOCAL_PROVIDER_BASE_URL` | If using Local Provider | Base URL for LM Studio / Ollama OpenAI-compatible endpoint    |
